...
assert_facts([], _).
assert_facts([F1 | Facts], Name) :- 
    Name == 1 -> ( assert(queried(F1)),  assert_facts(Facts, Name)).
assert_facts([[F1, F2 | _ ] | Facts], Name) :-  
    Name == 2 -> ( assert(type(F1, F2)), assert_facts(Facts, Name) );
        Name == 3-> ( assert(grade(F1, F2)), assert_facts(Facts, Name) ); nl.
...
start() :- 
    ...
    findall(Interest, py_interests(Interest), InterestList), 
    findall(X, py_semester(X), [Semester|_]),
    findall(Y, py_stream(Y), [Stream|_]),
    findall(Z, py_cgpa(Z), [CGPA|_]),
    findall(C, py_corecredits(C), [CoreCredits|_]),

    findall(Q, py_queried(Q), Queried),
    findall([C1, T], py_type(C1, T), Types),
    findall([C2, G], py_grade(C2, G), Grades),

    %now assert these facts
    assert_facts(Queried,  1), 
    assert_facts(Types, 2), 
    assert_facts(Grades, 3),
    ...
    (Stream > 2 -> 
        findall(S, py_spzlcredits(S), [SpecCredits|_]); nl),
